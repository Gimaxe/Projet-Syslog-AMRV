...
## Installation et configuration du Serveur Web
  - ### MariaDB
    ...
    - #### Configuration
      ```bash
      mysql_secure_installation # Lors de cette étape, uniquement changer le MdP du root
      ```
  - ### Nginx
    - #### Installation
      ```bash
    ...
    - #### Configuration
      ```bash
      cd /etc/nginx/sites-enabled # Emplacement des fichier des instances web de Nginx
      sudo vim nextcloud # Créer le fichier de conf nextcloud 
      ```
      ```bash
      # Copier ces lignes dans le fichier de conf
      # Version 2024-07-17
  ...
  - ### Nextcloud
    - #### Installation
      ```bash
      cd /srv # Emplacement des fichier de l'app
      sudo curl -O https://download.nextcloud.com/server/releases/latest.zip # Téléchargement des fichier de Nextcloud
      sudo apt install unzip # Installation de UnZip (Pas installé par défaut)
      sudo unzip latest.zip # Décompression des fichier de Nextcloud
      ```